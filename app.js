...
async function api(action, payload={}) {
  if (CONFIG.DEMO_MODE || CONFIG.API_URL.includes("PASTE_")) return demoApi(action, payload);
  const response = await fetch(CONFIG.API_URL, {
    method:"POST", headers:{"Content-Type":"text/plain;charset=utf-8"},
    body:JSON.stringify({action, token:localStorage.getItem("dsnToken") || "", ...payload})
  });
...
function demoApi(action, payload) {
  const demoStudents = JSON.parse(localStorage.getItem("demoStudents") || "null") || [
    {studentId:"S001", numberInClass:1, firstName:"กิตติพงศ์", lastName:"ใจดี", classCode:"M3-9", teamStatus:"Joined", locationStatus:"Submitted", latitude:13.944, longitude:100.397, district:"บางบัวทอง", address:"88/12 บางบัวทอง นนทบุรี"},
    {studentId:"S002", numberInClass:2, firstName:"พิมพ์ชนก", lastName:"แสงทอง", classCode:"M3-9", teamStatus:"Joined", locationStatus:"NotSubmitted"},
    {studentId:"S003", numberInClass:3, firstName:"ธนภัทร", lastName:"รักษ์ดี", classCode:"M3-9", teamStatus:"Joined", locationStatus:"Submitted", latitude:13.926, longitude:100.415, district:"บางใหญ่", address:"12/4 บางใหญ่ นนทบุรี"},
    {studentId:"S004", numberInClass:4, firstName:"ณัฐชา", lastName:"วงศ์งาม", classCode:"M3-9", teamStatus:"Joined", locationStatus:"Submitted", latitude:13.896, longitude:100.424, district:"บางแม่นาง", address:"9/21 บางแม่นาง นนทบุรี"},
    {studentId:"S005", numberInClass:5, firstName:"ภูริณัฐ", lastName:"ศรีสุข", classCode:"M3-9", teamStatus:"Joined", locationStatus:"Submitted", latitude:13.865, longitude:100.412, district:"บางกรวย", address:"44/8 บางกรวย นนทบุรี"}
  ];
  if (action === "login") {
    const role = payload.role;
    return Promise.resolve({ok:true, token:"demo-token", user:{userId:"DEMO", email:payload.email, role, profileCompleted:true}, profile: role==="student"
      ? {firstName:"นักเรียน", lastName:"ตัวอย่าง", classCode:"M3-9", numberInClass:6, locationStatus:localStorage.getItem("demoLocation")?"Submitted":"NotSubmitted", teacherName:"ครูพีพี"}
      : role==="teacher" ? {firstName:"พัชรพงศ์", lastName:"ตรองจิตต์", classCode:"M3-9"} : {firstName:"ผู้ดูแลระบบ", lastName:""}});
  }
  if (action === "register") return Promise.resolve({ok:true, needsProfile:true, token:"demo-token", user:{role:payload.role,email:payload.email}});
  if (action === "saveProfile") return Promise.resolve({ok:true, profile:payload.profile});
  if (action === "saveLocation") { localStorage.setItem("demoLocation",JSON.stringify(payload.location)); return Promise.resolve({ok:true}); }
  if (action === "getLocation") return Promise.resolve({ok:true, location:JSON.parse(localStorage.getItem("demoLocation") || "null")});
  if (action === "getTeacherDashboard") return Promise.resolve({ok:true, students:demoStudents, groups:JSON.parse(localStorage.getItem("demoGroups") || "[]")});
  if (action === "calculateGroups") return Promise.resolve({ok:true, groups:clusterStudents(demoStudents.filter(s=>s.locationStatus==="Submitted"), payload.groupSize || 4)});
  if (action === "saveGroups") { localStorage.setItem("demoGroups",JSON.stringify(payload.groups)); return Promise.resolve({ok:true}); }
  if (action === "forgotPassword") return Promise.resolve({ok:true});
  if (action === "getAdminDashboard") return Promise.resolve({ok:true, academicYear:state.academicYear, metrics:{users:32,teachers:2,students:30,mapped:24}, accounts:[
    {email:"teacher@example.com",role:"teacher",name:"ครูพัชรพงศ์ ตรองจิตต์",classCode:"M3-9",status:"Active"},
    {email:"student@example.com",role:"student",name:"นักเรียน ตัวอย่าง",classCode:"M3-9",status:"Active"}]});
  if (action === "setAcademicYear") { state.academicYear=payload.year; return Promise.resolve({ok:true});}
  if (action === "toggleAccount") return Promise.resolve({ok:true});
  return Promise.resolve({ok:true});
}

function clusterStudents(students, size) {
  const school = CONFIG.SCHOOL_COORDS;
  const list = students.map(s=>({...s, angle:Math.atan2(s.latitude-school[0], s.longitude-school[1])})).sort((a,b)=>a.angle-b.angle);
  const groups=[]; for(let i=0;i<list.length;i+=size){ const members=list.slice(i,i+size); groups.push({
    groupId:`G${Date.now()}-${groups.length+1}`, groupName:`กลุ่มที่ ${groups.length+1}`, members:nearestNeighbor(members, school)
  });}
  return groups;
}
function nearestNeighbor(items, start) {
  const remain=[...items], ordered=[]; let current={latitude:start[0],longitude:start[1]};
  while(remain.length){ let best=0,bestD=Infinity; remain.forEach((x,i)=>{const d=(x.latitude-current.latitude)**2+(x.longitude-current.longitude)**2;if(d<bestD){bestD=d;best=i}});
    current=remain.splice(best,1)[0]; ordered.push(current);
  } return ordered;
}
...
$("locationForm").addEventListener("submit",async e=>{
  ...
  const location={houseNumber:$("houseNumber").value,moo:$("moo").value,village:$("village").value,soi:$("soi").value,road:$("road").value,
    ...
    contactName:$("contactName").value,contactPhone:$("contactPhone").value,preferredDate:$("preferredDate").value,preferredTime:$("preferredTime").value,
